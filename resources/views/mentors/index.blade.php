            ...
            @foreach ($mentors as $mentor)
                <div class="w-full md:w-1/3 px-4 mb-8">
                    <div class="bg-white rounded-lg overflow-hidden shadow-lg">
                        <img class=" h-48 object-cover object-center w-full" src="{{ asset('images/'.$mentor->photo) }}" alt="{{ $mentor->name }}">
                        <div class="px-6 py-4">
                            <h2 class="text-lg font-medium text-gray-800 mb-1 text-center">{{ $mentor->name }}</h2>
                            <p class="text-sm text-gray-600 mb-1 text-center">{{ $mentor->profession }}</p>
                            <p class="text-gray-700 text-base text-center">{{ $mentor->description }}</p>
